// Source:
// https://github.com/sahaj-b/ghostty-cursor-shaders/blob/main/rectangle_boom_cursor.glsl
// CONFIGURATION
const float DURATION = 0.10; // How long the ripple animates (seconds)
const float MAX_SIZE =
    0.01; // Max radius in normalized coords (0.5 = 1/4 screen height)
const float ANIMATION_START_OFFSET =
    0.0; // Start the ripple slightly progressed (0.0 - 1.0)
vec4 COLOR = vec4(0.35, 0.36, 0.44,
                  1.0); // change to iCurrentCursorColor for your cursor's color
...
float sinPulse(float t) { return sin(t * 3.1415916); }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // why? because its UNIFORM branching (ie, all fragments take the same path)
  // which modern GPUs handles efficiently its far more efficient than
  // calculating the ripple EVERY FRAME even when not needed(branchless)
  if (isModeChange > 0.0 && isAnimating > 0.0) {
    ...
    vec2 halfSizeCC =
        vec2(currentCursor.z, currentCursor.w) * 0.5 + vec2(rippleExpansion);
    ...
    float ripple =
        (1.0 - smoothstep(-antiAliasSize, antiAliasSize, sdfRectRing)) * fade;
    // Apply ripple effect
